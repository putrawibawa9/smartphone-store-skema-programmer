 ...
 public static function decreseStock($id, $quantity){
    ...
    public static function find($id)
    {
      ...
      public static function search($nama){
        $instance = new static();
        $instance->db->query('SELECT * FROM ' . static::$table .' WHERE nama LIKE :nama');
        $instance->db->bind(':nama', "%$nama%");
        return $instance->db->resultSet();
    }
...
// prepare query based on its table
$this->db->query('SELECT password FROM ' . static::$table . ' WHERE username=:username');
